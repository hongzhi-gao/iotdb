...
public:
  ...
  void addValue(size_t schemaId, size_t rowIndex, const T &value) {
    ...
    case TSDataType::DATE: {
      safe_cast<T, boost::gregorian::date>(
          value, ((boost::gregorian::date *)values[schemaId])[rowIndex]);
      break;
    }
  ...
  void *getValue(size_t schemaId, size_t rowIndex,
    ...
    case TSDataType::DATE:
      return &(reinterpret_cast<boost::gregorian::date *>(
          values[schemaId])[rowIndex]);
    case TSDataType::TIMESTAMP:
    case TSDataType::INT64:
...
private:
  ...
  using EndPointSessionMap =
      std::unordered_map<TEndPoint, shared_ptr<SessionConnection>,
...
private:
  ...
  void insertTabletsWithLeaderCache(unordered_map<string, Tablet *> tablets,
  ...
  void setDatabase(const std::string &database) { this->database_ = database; }

  string getDatabase() { return database_; }

  void changeDatabase(string database) { this->database_ = database; }
