...
void Tablet::deepCopyTabletColValue(void *const *srcPtr, void **destPtr,
  ...
  case TSDataType::DATE: {
    *destPtr = new boost::gregorian::date[maxRowNumber];
    boost::gregorian::date *srcDate =
        static_cast<boost::gregorian::date *>(src);
    boost::gregorian::date *destDate =
        static_cast<boost::gregorian::date *>(*destPtr);
...
string SessionUtils::getValue(const Tablet &tablet) {
  ...
  for (size_t i = 0; i < tablet.schemas.size(); i++) {
    ...
    case TSDataType::DATE: {
      boost::gregorian::date *valueBuf =
          (boost::gregorian::date *)(tablet.values[i]);
      for (size_t index = 0; index < tablet.rowSize; index++) {
        if (!bitMap.isMarked(index)) {
...
void Session::removeBrokenSessionConnection(
...
void Session::sortTablet(Tablet &tablet) {
  ...
  for (size_t i = 0; i < tablet.schemas.size(); i++) {
    ...
    case TSDataType::DATE: {
      sortValuesList((boost::gregorian::date *)(tablet.values[i]), index,
                     tablet.rowSize);
      break;
    }
    case TSDataType::TIMESTAMP:
...
void Session::sortIndexByTimestamp(int *index, std::vector<int64_t> &timestamps,
                                   int length) {
  if (length <= 1) {
    return;
  }
...
void Session::insertStringRecordsWithLeaderCache(
...
void Session::insertRecordsWithLeaderCache(
    vector<string> deviceIds, vector<int64_t> times,
...
void Session::insertTabletsWithLeaderCache(
...
void Session::open() { open(false, DEFAULT_TIMEOUT_MS); }
...
void Session::open(bool enableRPCCompression, int connectionTimeoutInMs) {
  ...
  if (enableRedirection_) {
    endPointToSessionConnection.insert(
        make_pair(defaultEndPoint_, defaultSessionConnection_));
  }
...
void Session::insertRecords(
  ...
  if (enableRedirection_) {
    insertRecordsWithLeaderCache(deviceIds, times, measurementsList, typesList,
                                 valuesList, false);
  } else {
    TSInsertRecordsReq request;
    request.__set_prefixPaths(deviceIds);
...
void Session::buildInsertTabletReq(TSInsertTabletReq &request, Tablet &tablet,
                                   bool sorted) {
...
void Session::insertRelationalTablet(Tablet &tablet, bool sorted) {
  ...
  if (tableModelDeviceIdToEndpoint.empty()) {
    ...
    relationalTabletGroup.insert(
        make_pair(getSessionConnection(tablet.getDeviceID(0)), tablet));
  } else {
    for (int row = 0; row < tablet.rowSize; row++) {
      auto iDeviceID = tablet.getDeviceID(row);
      std::shared_ptr<SessionConnection> connection =
          getSessionConnection(iDeviceID);
      ...
      if (it == relationalTabletGroup.end()) {
        Tablet newTablet(tablet.deviceId, tablet.schemas, tablet.columnTypes,
                         tablet.rowSize);
        it = relationalTabletGroup.insert(std::make_pair(connection, newTablet))
                 .first;
      }
      ...
      for (int col = 0; col < tablet.schemas.size(); col++) {
        ...
        case TSDataType::INT32:
          currentTablet.addValue(tablet.schemas[col].first, rowIndex,
                                 *(int32_t *)tablet.getValue(
                                     col, row, tablet.schemas[col].second));
          break;
        case TSDataType::INT64:
        case TSDataType::TIMESTAMP:
          currentTablet.addValue(tablet.schemas[col].first, rowIndex,
                                 *(int64_t *)tablet.getValue(
                                     col, row, tablet.schemas[col].second));
          break;
        case TSDataType::FLOAT:
          currentTablet.addValue(
        ...
        case TSDataType::DATE: {
          currentTablet.addValue(tablet.schemas[col].first, rowIndex,
                                 *(boost::gregorian::date *)tablet.getValue(
                                     col, row, tablet.schemas[col].second));
          break;
        }
        case TSDataType::STRING:
...
void Session::insertRelationalTabletByGroup(
  ...
  for (auto iter = relationalTabletGroup.begin();
       iter != relationalTabletGroup.end(); iter++) {
    auto connection = iter->first;
    auto tablet = iter->second;
...
void Session::testInsertRecords(const vector<string> &deviceIds,
  ...
  try {
    TSStatus tsStatus;
    defaultSessionConnection_->getSessionClient()->insertStringRecords(tsStatus,
                                                                       request);
    RpcUtils::verifySuccess(tsStatus);
  } catch (const TTransportException &e) {
    log_debug(e.what());
...
Session::getSessionConnection(std::string deviceId) {
  if (!enableRedirection_ ||
      deviceIdToEndpoint.find(deviceId) == deviceIdToEndpoint.end() ||
...
void Session::handleRedirection(const std::string &deviceId,
                                TEndPoint endPoint) {
  if (!enableRedirection_)
    return;
...
void Session::handleRedirection(
...
Session::executeRawDataQuery(const vector<string> &paths, int64_t startTime,
                             int64_t endTime) {
  return defaultSessionConnection_->executeRawDataQuery(paths, startTime,
                                                        endTime);
}
...
int Session::countMeasurementsInTemplate(const string &template_name) {
  ...
  TSQueryTemplateResp resp =
      defaultSessionConnection_->querySchemaTemplate(req);
  return resp.count;
}
